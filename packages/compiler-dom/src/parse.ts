export const enum NodeTypes {
	...
	ATTRIBUTE = 6,
	DIRECTIVE = 7,
	COMPOUND_EXPRESSION = 8,// 组合表达式 {{name}} hi,boy
...
function parseInterpolation(context){
	...
	const innerEnd = getCursor(context); // 这个end稍后会改，先拿到
...
function advancePositionWithMutation(context, s, endIndex) {
	...
	context.column = linePos == -1 ? context.column + endIndex : endIndex - linePos// 如何更新列数
}
